...
server.mount_proc '/details' do |req, res|
    ...
    begin
        ...
        ratings = check_ratings(id, headers)
        print "got raings #{ratings}"
        if ratings == "200" 
            ...
            res.body = {
                ...
                'language' => 'English',
                'ISBN-10' => 'Something went wrong with ratings',
            }.to_json
            res['Content-Type'] = 'application/json'
        end
    rescue => error
...
def check_ratings(id, headers)
    begin
        uri = URI.parse("http://ratings:9080/ratings/0")
        http = Net::HTTP.new(uri.host, 9080)
