...
def front():
    ...
    #    print("try again!!")
    #    detailsStatus, details = getProductDetailsBuggy(product_id, headers)
    ...
    if reviewsStatus == 200:
        ratings = getProductRatingsBuggy(product_id, headers)
...
def getProductDetailsBuggy(product_id, headers):
    #url = details['name'] + "/" + details['endpoint'] + "/" + str(product_id) + "?FOO"
    url = details['name'] + "/" + details['endpoint'] + "/1?FOO"
    #url = details['name'] + "/" + 'details2' + "/" + str(product_id) + "?FOO"

    headers["Cache-Control"] = "no-cache"
    res = requests.get(url, headers=headers, timeout=3.0)
    return 200, res.json()
    #return 200, {"error:": "you know you done fucked up"}


def getProductDetails(product_id, headers):
    try:
        #url = details['name'] + "/" + details['endpoint'] + "/" + str(product_id)
        url = details['name'] + "/" + details['endpoint'] + "/" + "2"
    ...
    if res and res.status_code == 200:
        ...
        return status, {'error': 'Sorry, product details are currently unavailable for this book, ya schnor'}
...
def getProductRatingsBuggy(product_id, headers):
    url = ratings['name'] + "/" + ratings['endpoint'] + "/" + str(product_id)
    res = requests.get(url, headers=headers, timeout=3.0)
    return 200, res.json()
